...
for i in range(size/2+1):
	...
	#draw.rectangle((size/2,size/2,size/2,size/2), outline=(int(r),int(g),int(b)))
img2.save("frames/0.png","PNG")
#img2.save("frames/40.png","PNG")
...
for y in range(height):
	...
	for x in range(width):
		...
		# saving croped  files to frames-half
		#new.save("frames-half/"+outputfile+str (count)+".png")
		#image = Image.open("frames-half/"+outputfile+str (count)+".png")
		#new_image = image.resize((500, 500))
		#text_img = Image.new('RGBA', (size,size), (0, 0, 0, 0))
		#text_img.paste(img2, (0,0))
		#text_img.paste(new_image, (0,0), mask=new_image)
		#text_img.save("frames-half/"+outputfile+str (count)+".png", format="png")
		#text_img.save("frames-half/"+outputfile+str ((width*2)-count)+".png")
...
#	image_file = ("frames-half/"+str (x)+'.png')

#frames_right_merged[0].save("wolf_r_merged.gif", save_all=True, append_images=frames_right_merged[0:], duration=1, loop=0)
